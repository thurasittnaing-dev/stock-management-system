...
@section('content')
    <div class="container-fluid">
        ...
        <div class="my-2 d-flex justify-content-between p-0">
            ...
            <div class="d-flex">
                <div>
                    <a href="{{ route('location.create') }}" class="btn btn-outline-primary btn-sm cus-btn">
            ...
            <tbody>
                @forelse ($locations as $location)
                    <tr>
                        ...
                        <td>
                            <span class="mr-1"> {{ date('d-m-Y', strtotime($location->created_at)) }}</span>
                            <small class="text-info">{{ $location->created_at->diffForHumans() }}</small>
                        </td>
                        <td>
                            <div class="d-flex">
                                <div class="mr-1">
                                    <a href="{{ url('admin/location/' . $location->id . '/edit?page=' . $page) }}"
                                ...
                                <form action="{{ route('location.destroy', $location->id) }}" method="POST"
                                    ...
                                    <button type="button" data-id="{{ $location->id }}"
                                        id="delete-btn-{{ $location->id }}" class="btn btn-sm btn-outline-danger"><i
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            // delete btn
            let locations = @json($locations).data;

            locations.forEach(location => {
                $(`#delete-btn-${location.id}`).on('click', function(e) {
                    let id = $(`#delete-btn-${location.id}`).attr('data-id');
                    // sweet alert
                    Swal.fire({
                        title: 'Are you sure?',
