...
@section('content')
    <div class="container-fluid">
        ...
        <div class="my-2 d-flex justify-content-between p-0">
            ...
            <div class="d-flex">
                <div>
                    <a href="{{ route('supplier.create') }}" class="btn btn-outline-primary btn-sm cus-btn">
            ...
            <tbody>
                @forelse ($suppliers as $supplier)
                   <tr>
                    ...
                    <td>
                      <span class="mr-1"> {{ date('d-m-Y', strtotime($supplier->created_at)) }}</span>
                      <small class="text-info">{{ $supplier->created_at->diffForHumans() }}</small>
                    </td>
                    <td>
                      <div class="d-flex">
                        <div class="mr-1">
                            <a href="{{ url('admin/supplier/' . $supplier->id . '/edit?page=' . $page) }}"
                        ...
                        <form action="{{ route('supplier.destroy', $supplier->id) }}" method="POST"
                            ...
                            <button type="button" data-id="{{ $supplier->id }}"
                                id="delete-btn-{{ $supplier->id }}" class="btn btn-sm btn-outline-danger"><i
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            // delete btn
            let suppliers = @json($suppliers).data;

            suppliers.forEach(supplier => {
                $(`#delete-btn-${supplier.id}`).on('click', function(e) {
                    let id = $(`#delete-btn-${supplier.id}`).attr('data-id');
                    // sweet alert
                    Swal.fire({
                        title: 'Are you sure?',
